...
export default function NewsTicker() {
  return (
    <section
      className="bg-theme-gray py-24 border-t border-gray-200"
      id="admissions"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          <div
            className="inline-flex items-center gap-2 px-4 py-2 bg-theme-red text-white text-sm font-semibold rounded-full shadow"
            role="alert"
          >
            <span className="w-2 h-2 bg-white rounded-full pulse-dot flex-shrink-0" aria-hidden="true" />
            🚀 New Beta Website is Live!
          </div>
          <div
            className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-50 text-yellow-800 text-sm font-semibold rounded-full border border-yellow-300 shadow"
            role="alert"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {newsItems.map((item, index) => (
            ...
            >
              <div
                className="h-28 bg-gradient-to-br from-red-50 to-pink-50 flex items-center justify-center text-5xl border-b border-gray-100"
                aria-hidden="true"
              >
                {item.icon}
              </div>
              <div className="p-5 flex flex-col flex-1">
                ...
                <a href="#" className="text-xs font-bold text-theme-red hover:underline self-start">
                  Read More →
                </a>
              </div>
            </article>
